...
#   4242:1000 - an arbitrary uid that only shares the steam group (orchestrators)
#
# Each must be able to write every directory odin's install preflight checks
# (`preflight_write_checks` in src/odin/server/install.rs) plus $HOME. Sudo is blocked with
# no-new-privileges, as it is under `allowPrivilegeEscalation: false`, so the entrypoint's
# chown fallback cannot paper over a bad image.
#
# Usage: .github/scripts/check-image-permissions.sh <image>
set -euo pipefail

IMAGE="${1:?usage: $0 <image>}"
USERS=("1000:1000" "111:1000" "4242:1000")
...
for user in "${USERS[@]}"; do
  if output="$(docker run --rm --user "${user}" --security-opt no-new-privileges \
    ...
    echo "ok   --user ${user} can write every preflight path"
  else
    echo "FAIL --user ${user}"
    echo "${output}"
    failures=$((failures + 1))
