...
| THUNDERSTORE_TOKEN             | `<nothing>`                            | FALSE    | Thunderstore service account API token (`tss_...`, created from your team's Service Accounts page). Sent as `Authorization: Bearer` to `thunderstore.io` and its subdomains. Note that mod listing, version resolution and downloads are public endpoints, so a token is optional and does **not** exempt you from rate limiting. |
