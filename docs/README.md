...
| THUNDERSTORE_TOKEN             | `<nothing>`                            | FALSE    | Optional. Thunderstore service account API token (`tss_...`, created from your team's Service Accounts page). Sent as `Authorization: Bearer` on requests to `thunderstore.io` and its subdomains. Takes precedence over `THUNDERSTORE_USERNAME`/`THUNDERSTORE_PASSWORD`.                                                          |
| THUNDERSTORE_USERNAME          | `<nothing>`                            | FALSE    | Optional, legacy. Combined with `THUNDERSTORE_PASSWORD`, sends HTTP Basic Auth on requests to `thunderstore.io` and its subdomains (mod listing, version resolution, and downloads). Both must be set. Prefer `THUNDERSTORE_TOKEN`.                                                                                                                                        |
...
| MAX_CONCURRENT_DOWNLOADS       | `4`                                    | FALSE    | Maximum simultaneous mod downloads (and range chunks) while `CONCURRENT_DOWNLOADS_ENABLED` is true. Ignored when concurrency is disabled.                                                                                                                                                                                         |
