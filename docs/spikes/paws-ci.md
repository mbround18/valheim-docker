...
5. **Docker build args from the CLI. Added in paws `v0.0.1-prerelease.49`.** paws used to read build args only from a `compose.yml` service, so `GITHUB_SHA`, `GITHUB_REF` and `GITHUB_REPOSITORY` went unset and `/home/steam/.version` read `not-set`. mbround18/paws#34 added `--build-arg`, which `images.yml` passes. `ODIN_IMAGE_VERSION` was never declared as an `ARG`, so dropping it changes nothing.
...
7. **Releasing is gated on the `release` label, as it was before.** `.autorc` set `onlyPublishWithReleaseLabel: true`, so a merge released only when its PR carried `release`; `major`/`minor`/`patch` only chose the size once a release was authorized. paws has no such gate — `paws semver` falls back to a patch bump when no size label matches — so `release.yml` now has a `gate` job that reads the merged PR's labels and stops unless one is `release`. Without it every merge would tag, publish and move `:latest`, including the Renovate bumps kodiak auto-merges. A `workflow_dispatch` run is the deliberate override. `.autorc` is deleted, since it's no longer read and was the only written record of the old policy.
8. **Binaries are built before anything is published.** A tag and a GitHub Release can't be withdrawn, and `images.yml` starts the moment the tag lands, so `release.yml` builds and smoke-tests the binaries with `--no-upload` first, then tags, then uploads. The upload step rebuilds against the cache the first build just filled. The two `paws semver` calls are compared, and the run fails if the version drifted between them.
9. **Published images keep their OCI labels. Added in paws `v0.0.1-prerelease.49`.** `docker-meta` set `org.opencontainers.image.source`, `.revision`, `.version`, `.created`, `.title`, `.description` and `.licenses` on every published image (confirmed on `mbround18/valheim:3.8.0` and its GHCR copy). paws set none, and GHCR uses `image.source` to link a package to its repository, so publishing through paws would have un-linked the packages and dropped provenance. mbround18/paws#34 added `--label` and `--build-arg`; `images.yml` passes the same set `docker-meta` did, and the build args restore `/home/steam/.version` (gap 5).
10. **Push behavior changes.**
