...
fn add_additional_args(args: &mut Vec<String>) {
  ...
  let cfg = BetaConfig::from_env();
  add_beta_args(args, cfg.use_public_beta, cfg.branch, cfg.password);
}
...
pub fn install(app_id: i64) -> io::Result<ExitStatus> {
  ...
  if let Err(e) = preflight_write_checks() {
  ...
  let maybe_stash = if environment::fetch_var("CLEAN_INSTALL", "0") == "1" {
...
fn preflight_write_checks() -> Result<(), String> {
...
mod tests {
  ...
  fn test_add_beta_args() {
    ...
    assert_eq!(
      args.join(" "),
      "example -beta public-test -betapassword yesimadebackups validate"
    );
